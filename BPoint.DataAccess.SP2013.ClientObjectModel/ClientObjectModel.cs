namespace BPoint.SharePoint.DataAccess.SP2013
{
    ...
    public class ClientOMHelper
    {
        ...
        string sp2013url = null;
        string sp2013ecthuburl = null;
        ...
        private void GetClientContext()
        {
            ...
            {
                clientContext = ClaimClientContext.GetAuthenticatedContext(this.sp2013url, 1000, 1000);
                //Thread.Sleep(3000);
            }
        }
        public ClientOMHelper(string sp2013url, string sp2013ecthuburl, bool initializeContext, string username, string password)
        {
            this.sp2013url = sp2013url;
            this.sp2013ecthuburl = sp2013ecthuburl;
        ...
        {
            get { return sp2013ecthuburl; }
            set
            {
                if (sp2013ecthuburl != value)
                {
                    sp2013ecthuburl = value;
                    InitializeClientContext();
                }
            }
        }
        ...
        private Microsoft.SharePoint.Client.ClientContext ClientContext
        {
            get
            {
                if (clientContext == null && !string.IsNullOrEmpty(sp2013url))
                {
                    InitializeClientContext();
                }
        ...
        public List<CustomObjects.LookupValue> GetLookupValues(string listGuid, string showField, Guid webId)
        {
            ...
            else
            {
                ...
                {
                    ClientContext tempContext = new ClientContext(sp2013url);
        ...
        private static bool CheckIfQueryMustBePassed(SortedList<int, BPoint.SharePoint.Objects.WhereField> whereFieldsList)
        {
        ...
        private static void FormatCamlStringForDocumentsInCSharpForRestWithJson(string listName,
            SortedList<int, BPoint.SharePoint.Objects.ViewField> viewFieldsList,
            SortedList<int, BPoint.SharePoint.Objects.WhereField> whereFieldsList,
            XmlNode orderByNode,
            MainObject mainobject,
            ref StringBuilder sb)
        {
        ...
        private static void FormatRestSnippetInCSharpForViewfields(SortedList<int, BPoint.SharePoint.Objects.ViewField> viewFieldsList, ref Dictionary<string, string> restSnippets)
        {
            // simple example:
        ...
        private static bool FormatRestSnippetInCSharpForWhereFields(SortedList<int, BPoint.SharePoint.Objects.WhereField> whereFieldsList,
        ...
        private void InitializeClientContext()
        {
            try
            {
                if (string.IsNullOrEmpty(sp2013url))
                {
                ...
                // check the URL
                isSharePointOnline = sp2013url.Contains("microsoft.com") || sp2013url.Contains("sharepoint.com");
                ...
                {
                    clientContext = new Microsoft.SharePoint.Client.ClientContext(sp2013url);
                }
                ...
                if (!string.IsNullOrEmpty(sp2013ecthuburl))
                {
                    ecthubContext = new Microsoft.SharePoint.Client.ClientContext(sp2013ecthuburl);
        ...
        private void EnsureClientContext()
        {
            try
            {
                this.clientContext = new Microsoft.SharePoint.Client.ClientContext(sp2013url.Trim());
                ...
                this.clientContext.Credentials = new SharePointOnlineCredentials(username, passWord);
            }
            catch (Exception ex)
            {
        ...
        private void SetClientContextForSharePointOnline()
        {
            //clientContext = ClaimClientContext.GetAuthenticatedContext(sp2013url, 925, 525);
        }

        private void SetAuthenticatedCookiesForSharePointOnline()
        {
            //authCookies = ClaimClientContext.GetAuthenticatedCookies(sp2013url, 925, 525);
        }

        private void RefreshWeb(string webUrl)
        {
            if (this.sp2013url != webUrl)
            {
                EnsureClientContext();
                spWeb = ClientContext.Web;
                //ClientContext.Load(spWeb);
